...
let buttonClickHandler: ((id: string, isValid: boolean, values: any, form: any) => void) | null = null;
...
let tableActionHandler: ((actionId: string, value: any, row: any) => void) | null = null;
...
export function invokeActionClickHandler(
  action: string,
  row: any
) {
...
let checkedClickHandler: ((action: string, value: any, row: any) => void) | null = null;
...
let formFieldOnChangeHandler: ((field: string, value: any, form_value: any) => void) | null = null;
 ...
 */
export function autoMapRowToFormFields(row: any, formConfig: any): Record<string, any> {
  console.log("🔍 autoMapRowToFormFields called with:", { row, formConfig });

  if (!formConfig?.sections || !row) {
    console.warn("❌ Invalid formConfig or row data for auto-mapping", { formConfig, row });
    return {};
  }
    ...
    if (section.fields) {
      section.fields.forEach((field: any, fieldIndex: number) => {
        ...
        if (fieldName && row.hasOwnProperty(fieldName)) {
          ...
          switch (field.type) {
            case 'subform':
              ...
              if (Array.isArray(value)) {
                ...
                console.log(`✅ Mapped subform ${fieldName} with ${value.length} entries:`, value);
              } else {
                console.warn(`⚠️ Subform ${fieldName} expected array but got:`, typeof value, value);
                formData[fieldName] = [];
              }
              break;
            case 'boolean':
          ...
          console.log(`⚠️ Field ${fieldName} not found in row, using default:`, defaultValue);
        }
      });
    }
  });
...
export function validateTableFormAlignment(sampleRow: any, formConfig: any) {
  ...
  const missingInTable = Array.from(formFields).filter(field => !tableFields.has(field));
  const extraInTable = Array.from(tableFields).filter(field => !formFields.has(field));
  ...
  const result = {
    ...
    missingInTable,
    extraInTable,
    message: missingInTable.length === 0
                ...
                <div class="section">
                    ...
                    ${section.fields ? section.fields.map((field: any) => `
                        <div class="field">
                            ...
                            <div class="field-value">${values[field.name] || '-'}</div>
                        </div>
                    `).join('') : ''}
                </div>
            `).join('')}
